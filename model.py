...
class Model:
    ...
    def update(self, data: List[np.ndarray], intrvs: List[np.ndarray]):
        ...
        alpha_mu, alpha_W, nu, T, _, B = self.alpha_mu, self.alpha_W, self.nu[:], np.linalg.inv(self.T_inv), *self.getLGM()
        N = sum(samples.shape[0] for samples in data)
        IB = np.linalg.inv(np.eye(B.shape[0]) - B)
        alpha_mu, alpha_W = alpha_mu + N, alpha_W + N

        means = [np.mean(data[i], 0) - IB@intrvs[i] for i in range(len(data))]
        covs = [np.sum((data[i] - IB@intrvs[i] - means[i])@(data[i] - IB@intrvs[i] - means[i]).T, 0) for i in range(len(data))]
        
        #if N > params[2].shape[0]: # invert entire T because it's more efficient than Woodbury; TODO: have not implemented Woodbury

        T += sum(covs) + sum([(data[i].shape[0]*self.alpha_mu/alpha_mu)*((means[i] - nu)@(means[i] - nu).T) for i in range(len(data))])
    ...
    """
    def getLGM(self) -> Tuple[np.ndarray, np.ndarray]:
